...
pairsOnly = TRUE # only include apps that exist in both datasets
...
if (version == '2017') {
  ...
  apps <- read_csv("data/2017_study/appInfo.csv") %>% 
    left_join(genre_grouping)
  
  # with the 2017 data, then note that 'unknown' was in title case
  # so change it to lower case to get code to run correctly
  hosts <- vroom("data/2017_study/appsWithHostsAndCompaniesLong.csv") %>% 
  ...
  hosts <- vroom("data_processed/appsWithHostsAndCompaniesLong.csv")
