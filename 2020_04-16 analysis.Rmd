...
apps2020 <- read_csv("data/apps.csv") %>%
  left_join(genre_grouping) %>% 
  filter(id %in% mapping$id2020)

apps2017 <- read_csv("data/2017_study/appInfo.csv") %>%  # if you want to do the 2017 one
  left_join(genre_grouping) %>% 
...
hosts2020 <- vroom("data_processed/appsWithHostsAndCompaniesLong.csv") %>%
  filter(id %in% mapping$id2020)

# if you want to run this with the 2017 data, then note that 'unknown' was in title case
# so change it to lower case to get code to run correctly
hosts2017 <- vroom("data/2017_study/appsWithHostsAndCompaniesLong.csv") %>% 
  mutate(company = ifelse(company == "Unknown", "unknown", company)) %>%
  filter(id %in% mapping$id2017)
...
It will save out a CSV file called appsWithHostsAndCompaniesLong with three columns: app id, domains ('hosts'), and company. 
Each row represents one domain identified in an app, so each app has as many rows in there as it has domains.
The company column is 'unknown' if the domain has no match in our JSON of tracker domains.

Note that the chunk option `eval=FALSE` means that this chunk will not be evaluated if you knit this document.
That is because this mapping takes a long time - if you do the full mapping with ~1m apps, then it'll take something like 6-10 hours on a high-end laptop.
There's probably an efficient way to do this as a simple left-join, but the current implementation is an iterative search which is probably less error-prone.
...
#then do the matching - WARNING! WITH HUNDREDS OF THOUSANDS OF APPS, THIS TAKES HOURS

uniqueHosts = hosts[!duplicated(hosts$hosts), ]
hostsTmp = hosts
hosts = uniqueHosts
...
lookup <- data.frame(company = hosts$company, row.names = hosts$hosts)
hosts <- hostsTmp %>%  mutate(company = lookup[hosts, ])

write_csv(hosts, "data_processed/appsWithHostsAndCompaniesLong.csv")
...
#write_csv(summary_tracker_hosts, "results/host_analysis/summary_tracker_hosts_per_app.csv")
```
...
head(known_trackers_info, 250) %>%
  select(-leaf_parent) %>%
  write_csv("results/host_analysis/top_250_tracker_hosts.csv")
...
head(unknown_hosts_info, 250) %>%
  write_csv("results/host_analysis/top_250_unknown_hosts.csv")
```
...
```
Ok, now we can calculate summary statistics
...
#write_csv(summary_company_count, "results/company_analysis/summary_company_count.csv")
```
...
write_csv(prevalence_owners_and_subsidiaries, "results/company_analysis/prevalence_root_parents_and_subsidiaries.csv")
...
write_csv(summary_company_count_by_super_genre_add_family, "results/company_analysis/summary_company_count_by_super_genre_add_family.csv")
...
# create a vector with the names to save them with
names_to_save <- names(by_super_genre) %>% 
  str_to_lower() %>% 
  str_replace_all(" ", "") %>% 
  str_replace_all("&", "_") %>% 
  str_c("company_prevalence_", .)

map2_dfr(names(by_super_genre), by_super_genre, count_company_presence_subset_of_apps) %>% 
  write_csv("results/company_analysis/by_super_genre_company_prevalence.csv")
...
count_company_presence_subset_of_apps("Family", family_apps) %>% 
  write_csv("results/company_analysis/family_company_prevalence.csv")
...
write_csv(summary_country_counts,"results/country_analysis/country_summary.csv")
...
count_country_presence_subset_of_apps <- function(tag, apps_to_process){
  ...
  #write_csv(count_country_presence_subset_of_apps, storage_location)
}

count_country_presence_subset_of_apps(tag = "all apps", apps) %>% 
  write_csv("results/country_analysis/count_country_presence_subset_of_apps.csv")
...
write_csv(summary_country_count_by_super_genre_add_family, "results/country_analysis/by_super_genre_add_family_summary_country_count.csv")
...
map2_dfr(names(by_super_genre), by_super_genre, count_country_presence_subset_of_apps) %>% 
  write_csv("results/country_analysis/by_super_genre_count_country_presence.csv")

# do it also for family apps
count_country_presence_subset_of_apps(tag = "Family", family_apps) %>% 
  write_csv("results/country_analysis/family_count_country_presence.csv")
```
